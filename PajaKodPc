...
while True:
    ...
    if "martyn" in odpoved:
        print("Pája: Ne! Topytle! Ma tšečtina se vypařuje kdiz o nem mluvýš!") 
    if "majkl" in odpoved:
    ...
    elif not odpoved.islower():
        print("Systém: Prosím, piš Pájovi malými písmeny.")
    elif "ě" in odpoved or "š" in odpoved or "č" in odpoved or "ř" in odpoved or "ž" in odpoved or "ý" in odpoved or "á" in odpoved or "í" in odpoved or "é" in odpoved or "ú" in odpoved or "ů" in odpoved:
        print("Systém: Prosím, piš Pájovi bez háčků a čárek.")
    elif not "." in odpoved and not "!" in odpoved and not "?" in odpoved:
        print("Systém: Prosím, ukonči větu tečkou, vykřičníkem nebo otazníkem.")
    elif odpoved == "jsi gay?":
        print("Pája: Já ne, ale Pepa z malého městečka pulhřimov ano.")
    ...
    elif any(word in odpoved for word in ["negr", "pic", "vole", "prdel", "kokot", "kurv", "zmrd", "hovno"]):
